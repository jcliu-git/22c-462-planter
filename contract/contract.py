...
class PhotoCaptureMessage(Message[PhotoCapture]):
    ...
    def __init__(self, filename: float, phototype: float, timestamp: Optional[datetime.datetime] = None):
        super().__init__(
            MessageType.DATA,
            System.CAMERA,
    ...
    def fromJson(message: IMessage[PhotoCapture]):
        return PhotoCaptureMessage(
            message["data"]["filename"], message["data"]["phototype"], message["data"]["timestamp"]
        )